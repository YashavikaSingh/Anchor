// Use one consolidated message listener
...
const API_ENDPOINT = "http://localhost:5173/videos"; // Replace with your actual endpoint
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.action) {
    ...
    case "videoChanged":
      console.log("Background received videoChanged", message.data);
    ...
    case "getCurrentVideo":
      console.log("Background received getCurrentVideo", currentVideoData);
      sendResponse({ data: currentVideoData });
      break;

    case "analyzeVideo":
      console.log("Background received analyzeVideo");
      if (!currentVideoData) {
        sendResponse({
          success: false,
          error:
...
Keep your response concise and insightful.
    ...
    // Check and extract the analysis text
    if (
      result.candidates &&
      result.candidates[0] &&
      ...
      sendResponse({ success: true, cached: false, analysis });
    } else {
